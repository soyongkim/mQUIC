...
void QuicPathValidator::OnPathResponse(const QuicPathFrameBuffer& probing_data,
  ...
  if (self_address != path_context_->self_address()) {
    ...
    //std::cout << "[quic_path_validator] fail validation : " << self_address << " vs " << path_context_->self_address() << std::endl;
    return;
  }
...
void QuicPathValidator::SendPathChallengeAndSetAlarm() {
  ...
  if (!should_continue) {
    ...
    std::cout << "[quic_path_validator] The delegate doesn't want to continue the path validation." << std::endl;
    CancelPathValidation();
    return;
  }

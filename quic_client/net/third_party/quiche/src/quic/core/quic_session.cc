...
void QuicSession::Initialize() {
  ...
  //std::cout << "[quic_session] Init!" << std::endl;
...
void QuicSession::OnCanWrite() {
  ...
  //std::cout << "[quic_session] OnCanWrite()" << std::endl;
  if (!RetransmitLostData()) {
    ...
    QUIC_DVLOG(1) << ENDPOINT
                  << "Cannot finish retransmitting lost data, connection is "
                     "write blocked.";
    //std::cout << "[quic_session] end OnCanWrite()1" << std::endl;
    return;
  }
  ...
  if (num_writes == 0 && !control_frame_manager_.WillingToWrite() &&
    ...
    //std::cout << "[quic_session] end OnCanWrite()2" << std::endl;
    return;
  }
  ...
  if (QuicVersionUsesCryptoFrames(transport_version())) {
    ...
    if (crypto_stream->HasBufferedCryptoFrames()) {
      ...
      // blocked.
      //std::cout << "[quic_session] end OnCanWrite()3" << std::endl;
      return;
    }
  }
  ...
  if (!datagram_queue_.empty()) {
    ...
    if (!datagram_queue_.empty()) {
      //std::cout << "[quic_session] end OnCanWrite()4" << std::endl;
      return;
    }
  }
  ...
  for (size_t i = 0; i < num_writes; ++i) {
    ...
    if (stream != nullptr && !stream->IsFlowControlBlocked()) {
      ...
      QUIC_DVLOG(1) << ENDPOINT << "stream " << stream->id()
                    ...
                    << previous_fin_sent;
      //std::cout << "[quic_session] hey stream on can write !" << std::endl;
      stream->OnCanWrite();
  ...
  //std::cout << "[quic_session] end OnCanWrite()" << std::endl;
}
...
void QuicSession::ProcessUdpPacket(const QuicSocketAddress& self_address,
  ...
  //std::cout << "[quic_session] process udp packet " << self_address << " " << peer_address << std::endl;
...
bool QuicSession::WriteControlFrame(const QuicFrame& frame,
                                    TransmissionType type) {
  //std::cout << "[quic_session] write control frame" << std::endl;
...
void QuicSession::ResetStream(QuicStreamId id, QuicRstStreamErrorCode error) {
  ...
  //std::cout << "[quic_connection] write control frame" << std::endl;
...
void QuicSession::RetransmitFrames(const QuicFrames& frames,
                                   TransmissionType type) {
  //std::cout << "[quic_session] retransmit frames" << std::endl;
...
bool QuicSession::RetransmitLostData() {
  //std::cout << "[quic_session] RetransmitLostData()" << std::endl;
  QuicConnection::ScopedPacketFlusher retransmission_flusher(connection_);
  ...
  //std::cout << "[quic_session] End RetransmitLostData()" << std::endl;

...
class QuicToyClient {
 public:
  ...
  int Simulate(std::vector<std::string> urls);

 private:
  ClientFactory* client_factory_;  // Unowned.
  int tracker_thread_kill_;
  uint64_t preLa = 0;
  //int64_t preBW, curBW;
};

class NetworkChangeConfig {
  public:
    int ho_num, ho_interval;
    int start_iface;
    std::string ho_case;

    NetworkChangeConfig();

    NetworkChangeConfig(int ho_num, int ho_interval, int start_iface, std::string ho_case) 
      : ho_num(ho_num), ho_interval(ho_interval), start_iface(start_iface), ho_case(ho_case) {}
};

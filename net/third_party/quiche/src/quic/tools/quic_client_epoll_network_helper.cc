...
bool QuicClientEpollNetworkHelper::CreateUDPSocketAndBind(
  ...
  testaddr.sin_port = htons(server_address.port());
  testaddr.sin_addr = server_address.host().GetIPv4();
  if(connect(fd, (struct sockaddr *)&testaddr, sizeof(testaddr)) < 0) {
    perror("[quic_client_epoll_network_helper] Fail connect()");
    close(fd);
    return false; 
  }
...
void QuicClientEpollNetworkHelper::RunEventLoop() {
  ...
  //std::cout << "[quic_cclient_epoll_network_helper] run loop!" << std::endl;

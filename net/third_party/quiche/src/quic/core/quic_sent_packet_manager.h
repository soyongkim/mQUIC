...
class QUIC_EXPORT_PRIVATE QuicSentPacketManager {
 public:
  ...
  // [sd] For close timer
  const QuicTime::Delta GetRetransmissionDelayForTimer(uint32_t dev);
  ...
  void SetPathValidationRtt(QuicTime::Delta pv_rtt) {
    pv_rtt_ = pv_rtt;
  }

  void initMeasureState() {
    pacing_sender_.SetMeasure();
  }

  void SetRetransmittedPacket(QuicByteCount ret_packet) {
    pacing_sender_.retransmittedPacket(ret_packet);
  }

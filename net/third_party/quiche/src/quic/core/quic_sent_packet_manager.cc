...
QuicSentPacketManager::QuicSentPacketManager(
    ...
    : unacked_packets_(perspective),
      ...
      pv_rtt_(QuicTime::Delta::FromMilliseconds(0)) {
...
void QuicSentPacketManager::MaybeInvokeCongestionEvent(
  ...
  if (using_pacing_) {
    //std::cout << "[quic_sent_packet_manager] MaybeInvokeCongestionEvent - using_pacing_" << std::endl;
  ...
  packets_lost_.clear();
  if (network_change_visitor_ != nullptr) {
    //std::cout << "[quic_sent_packet_manager] MaybeInvokeCongestionEvent2" << std::endl;
    network_change_visitor_->OnCongestionChange();
  }
}
...
void QuicSentPacketManager::MarkPacketHandled(QuicPacketNumber packet_number,
  ...
  //std::cout << "[quic_sent_packet_manager] remove in flight!" << std::endl;
...
QuicSentPacketManager::OnRetransmissionTimeout() {
  ...
  //std::cout << "[quic_sent_packet_manager] onRetransmitssionTimeout() : " << GetRetransmissionMode() << std::endl;
  switch (GetRetransmissionMode()) {
    ...
    case LOSS_MODE: {
      ...
      //std::cout << "[quic_sent_packet_manager] RTO" << std::endl;
...
void QuicSentPacketManager::RetransmitRtoPackets() {
  ...
  //std::cout << "[quic_sent_packet_manager] Retransmit RTO packets" << std::endl;
  ...
  if (!unacked_packets_.empty()) {
    ...
    for (; packet_number <= largest_sent_packet; ++packet_number) {
      ...
      if (transmission_info->state == OUTSTANDING &&
        ...
        QUICHE_DCHECK(transmission_info->in_flight);
        //std::cout << "[quic_sent_packet_manage] probing?? -" << packet_number << std::endl;
...
const QuicTime::Delta QuicSentPacketManager::GetRetransmissionDelayForTimer(uint32_t dev) {
  return QuicTime::Delta::FromMicroseconds(GetRetransmissionDelay().ToMicroseconds()/dev);
}

const QuicTime::Delta QuicSentPacketManager::GetPathDegradingDelay() const {
  //std::cout << "[quic_sent_packet_manager] num_ptos_for_path_degrading: " << num_ptos_for_path_degrading_ << std::endl;
...
const QuicTime::Delta QuicSentPacketManager::GetProbeTimeoutDelay(
  ...
  QuicTime::Delta pto_delay =
      ...
      (ShouldAddMaxAckDelay(space) ? peer_max_ack_delay_
                                   : QuicTime::Delta::Zero());
 // std::cout << "[quic_sent_packet_manager] srtt(" << rtt_stats_.smoothed_rtt() << ") + " << pto_rttvar_multiplier_ << "*" << rtt_var << " + " << peer_max_ack_delay_ << " = " << pto_delay << std::endl;
  pto_delay =
  ...
  //std::cout << "[quic_sent_packet_manager] pto delay: " << pto_delay << std::endl;
  return pto_delay;
}
...
QuicSentPacketManager::OnConnectionMigration(bool reset_send_algorithm) {
  ...
  if (!reset_send_algorithm) {
    ...
    // send_algorithm_->SetInitialCongestionWindowInPackets(pre_cwind);
    // std::cout << "[quic_sent_packet_manager] current cwind: " << send_algorithm_->GetCongestionWindow()/kDefaultTCPMSS << std::endl;
    return nullptr;
  }

...
class AckAlarmDelegate : public QuicConnectionAlarmDelegate {
 public:
  ...
  void OnAlarm() override {
    ...
    //std::cout << "[quic_connection] ACK timeout - me: " << connection_->self_address() << std::endl;
...
class ProcessUndecryptablePacketsAlarmDelegate
 ...
 public:
  ...
  void OnAlarm() override {
    QUICHE_DCHECK(connection_->connected());
    QuicConnection::ScopedPacketFlusher flusher(connection_); 
   // std::cout << "[quic_connection] ProcessUndecryptablePacketsAlarmDelegate" << std::endl;
...
// make bandwidth unit
double QuicConnection::BandwidthUnit() {
  double divisor;
  if (bw_ < 8 * 1000 * 1000) {
    // k
    divisor = 1e3;
  } else if (bw_ < INT64_C(8) * 1000 * 1000 * 1000) {
    // M
    divisor = 1e6;
  } else {
    // G
...
void QuicConnection::SetFromConfig(const QuicConfig& config) {
  ...
  if (config.HasReceivedMaxPacketSize()) {
    ...
    //std::cout << "[quic_connection] SetFromConfig => MaybeUpdatePacketCreatorMaxPacketLengthAndPadding()" << std::endl;
...
bool QuicConnection::MaybeTestLiveness() {
  ...
  //std::cout << "[quic_connnection] idle timeout: " << timeout << std::endl;
...
bool QuicConnection::OnPacketHeader(const QuicPacketHeader& header) {
  ...
  if (perspective_ == Perspective::IS_CLIENT) {
    if (!GetLargestReceivedPacket().IsInitialized() ||
      ...
      // number space.
      //std::cout << "[quic_connection] last me: " << last_received_packet_info_.destination_address << std::endl;
...
// [SD] timestamp
uint64_t QuicConnection::timeStamp() {
  return std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
}

QuicSocketAddress checkAddress;
bool QuicConnection::OnStreamFrame(const QuicStreamFrame& frame) {
  ...
  //std::cout << "[quic_connection] received stream (" << frame.offset << ")" << std::endl;

  //std::cout << "[quic_connection] idle timeout: " << idle_network_detector_.idle_network_timeout() << std::endl;

  // std::cout << "[quic_connection] Has in flight packet? " << sent_packet_manager_.HasInFlightPackets() 
  //             << " Has pending ack? " << HasPendingAcks() << std::endl;
  ...
  if(checkAddress.host() == last_received_packet_info_.destination_address.host()) {
    ...
    //   std::cout << "but port is different from " << checkAddress.port() << " to " << last_received_packet_info_.destination_address.port() << std::endl;
    // }

  } else {
    //std::cout << "[quic_connection] ho_start_ = " << ho_start_ << std::endl;
    ho_delay_ = timeStamp() - ho_start_;
    //ho_start_ = timeStamp();
...
bool QuicConnection::OnCryptoFrame(const QuicCryptoFrame& frame) {
  ...
  //std::cout << "[quic_connection] Received CRYPO frame from " << default_path_.peer_address << std::endl;
...
bool QuicConnection::OnAckFrameStart(QuicPacketNumber largest_acked,
  ...
  // std::cout << "[quic_connection] Received ACK Frame LA: " << largest_acked << "(" << largest_acked.ToUint64() - preAck << ")  ack delay: " << ack_delay_time
  //           << " My LA: " << ack_frame().largest_acked
  //           << std::endl;
  preAck = largest_acked.ToUint64();
...
bool QuicConnection::OnAckRange(QuicPacketNumber start, QuicPacketNumber end) {
  ...
  if (GetLargestReceivedPacketWithAck().IsInitialized() &&
      last_header_.packet_number <= GetLargestReceivedPacketWithAck()) {
    //std::cout << "[quic_connection] Received an old ack frame: ignoring" << std::endl;;
    QUIC_DLOG(INFO) << ENDPOINT << "Received an old ack frame: ignoring";
    return true;
  }
...
bool QuicConnection::OnAckFrameEnd(QuicPacketNumber start) {
  ...
  //std::cout << "[quic_connection] on acked end() and post process ack frame()" << std::endl;
...
bool QuicConnection::OnPathChallengeFrame(const QuicPathChallengeFrame& frame) {
  ...
  if(mquic_cwnd_size > 0) {
    // use the option of FLAGS_new_connection_measure in the server.
    // std::cout << "[quic_connection] Set handover adjustment factor: 1/" << mquic_cwnd_size << std::endl;
    sent_packet_manager_.mquic_cwnd_size = mquic_cwnd_size;
  }

  // [SD] cwnd override
  //sent_packet_manager_.cwind_override = true;

  std::cout << "[quic_connection] Received PC - last ip: " << last_received_packet_info_.destination_address <<"  cur ip: "<< default_path_.self_address << " - " << timeStamp() - ho_start_ << " msec" << std::endl;
...
bool QuicConnection::OnPathChallengeFrameInternal(
  ...
  if (should_proactively_validate_peer_address_on_path_challenge_) {
    ...
    std::cout << "[quic_connection] Start reverse path validation" << std::endl;
  ...
  // alternative path.
  //std::cout << "[quic_connection] send path response" << std::endl;
...
QuicTime pv_start = QuicTime::Zero();
QuicTime pv_end = QuicTime::Zero();
bool QuicConnection::OnPathResponseFrame(const QuicPathResponseFrame& frame) {
  ...
  if (!UpdatePacketContent(PATH_RESPONSE_FRAME)) {
    std::cout << "[quic_connection] PR update" << std::endl;
    return false;
  }
  if (debug_visitor_ != nullptr) {
    debug_visitor_->OnPathResponseFrame(frame);
    //std::cout << "[quic_connection] PR debug_visitor" << std::endl;
  }
  MaybeUpdateAckTimeout();
...
bool QuicConnection::OnConnectionCloseFrame(
  ...
  if(perspective_ == Perspective::IS_SERVER) {
    // measure connection migration
    //std::cout << "[quic_connection] Server Connection Close - cid: " << GetOneActiveServerConnectionId() << std::endl;
    sent_packet_manager_.initMeasureState();
    std::fstream writer;
    writer.open("cc_log.txt", std::ios::app);
    writer << std::endl;
    writer.close();
    ...
    std::cout << "[quic_connection] Received CC from client, Connection Closed.. " << std::endl;
  }
...
bool QuicConnection::ShouldSetRetransmissionAlarmOnPacketSent(
  ...
  if (!retransmission_alarm_->IsSet()) {
    //std::cout << "[quic_connection] ret is not set why?" << std::endl;
    return true;
  }
  if (!in_flight) {
    return false;
...
bool QuicConnection::OnNewConnectionIdFrame(
  ...
  //std::cout << "[quic_connection] Received NEW_CONNECTION_ID frame, so it is able to do next CM" << std::endl;
  ho_state_ = true;
...
bool QuicConnection::OnNewTokenFrame(const QuicNewTokenFrame& frame) {
  ...
  //std::cout << "[quic_connection] Received new token " << timeStamp() - ho_start_ << " msec" << std::endl;

  // if(ho_start_ != 0) {
  //   //std::cout << "[quic_connection] Received NEW_TOKEN frame - Handover Delay is " << timeStamp() - ho_start_ << " msec" << std::endl;
  //   ho_delay_ = timeStamp() - ho_start_;
  //   ho_start_ = 0;

  //   std::fstream writer;
  //   std::cout << "[quic_toy_client] Handover Delay : " << ho_delay_ <<  " ms" << std::endl;
  //   writer.open("ho_data.txt", std::ios::app);
  //   writer << ho_delay_ << std::endl;
  //   writer.close();
  // }
...
void QuicConnection::MaybeSendInResponseToPacket() {
  ...
  // connection to send.
  //std::cout << "[quic_connection] MaybeSendInResponseToPacket() -> HandleWriteBlocked()" << std::endl;
  ...
  if (defer_send_in_response_to_packets_) {
    ...
    //std::cout << "[quic_connection] MaybeSendInResponseToPacket() -> WriteIfNotBlocked()" << std::endl;
    WriteIfNotBlocked();
  }
}
...
uint64_t prePSN = 0;
QuicPacketCount preWindow(0);
uint64_t preSentTime = 0;
QuicStreamOffset pre_offset = 0;
QuicConsumedData QuicConnection::SendStreamData(QuicStreamId id,
  ...
  QuicConsumedData consumed_data(0, false);
  {
    ...
    if (perspective() == Perspective::IS_SERVER) {
        // QuicPacketCount diffwind = sent_packet_manager_.GetCongestionWindowInTcpMss() > preWindow ? sent_packet_manager_.GetCongestionWindowInTcpMss() - preWindow  : 0;
        // QuicStreamOffset diffoffset = offset > pre_offset?  offset - pre_offset : 0;
        // std::cout << "[quic_connection] send stream (" << diffoffset << ") - psn: "  << packet_creator_.NextSendingPacketNumber().ToUint64() << "(" << packet_creator_.NextSendingPacketNumber().ToUint64() - prePSN << ") - RetNum: " << stats_.packets_retransmitted 
        //   << " cwind: " << sent_packet_manager_.GetCongestionWindowInTcpMss() << "(" << diffwind << ")" 
        //   << " sRtt/lRtt: " << sent_packet_manager_.GetRttStats()->smoothed_rtt() << "/" << sent_packet_manager_.GetRttStats()->latest_rtt()
        //   << " pto/rto: " << sent_packet_manager_.GetPtoDelay() << "/" << sent_packet_manager_.GetRetransmissionDelayForTimer(1).ToMilliseconds()
        //   << " - " << timeStamp() - preSentTime << " msec" << std::endl;


        // pre_offset = offset;
        // preSentTime = timeStamp();
        // prePSN = packet_creator_.NextSendingPacketNumber().ToUint64();
        // preWindow = sent_packet_manager_.GetCongestionWindowInTcpMss();

        if(maxWindow < sent_packet_manager_.GetCongestionWindowInTcpMss() && window_flag) {
            maxWindow = sent_packet_manager_.GetCongestionWindowInTcpMss();
        }

        if(maxRet < stats_.packets_retransmitted) {
           maxRet = stats_.packets_retransmitted;
        }
    }

    ScopedPacketFlusher flusher(this);
        //std::cout << "[quic_connection] send stream data" << std::endl;
    consumed_data =
...
void QuicConnection::OnStreamReset(QuicStreamId id,
  ...
  if (packet_creator_.HasPendingStreamFramesOfStream(id)) {
    ScopedPacketFlusher flusher(this);
       // std::cout << "[quic_connection] on stream reset" << std::endl;
    packet_creator_.FlushCurrentPacket();
  }
...
void QuicConnection::ProcessUdpPacket(const QuicSocketAddress& self_address,
  ...
  // if (measure_nc_ > 0) {
  //   // use the option of FLAGS_new_connection_measure in the server.
  //   std::cout << "[quic_connection] Set cwind override - process udp" << std::endl;
  //   sent_packet_manager_.cwind_override = true;
  // }
  ...
  ScopedPacketFlusher flusher(this);
  //std::cout << "[quic_connection] process udpscoekt" << std::endl;
  ...
  if (!validate_client_addresses_ &&
      ...
      (!highest_packet_sent_before_effective_peer_migration_.IsInitialized() ||
       sent_packet_manager_.GetLargestObserved() >
           highest_packet_sent_before_effective_peer_migration_)) {
    if (perspective_ == Perspective::IS_SERVER) {
      //std::cout << "[quic_connection] 0" << std::endl;
  ...
  if (!MaybeProcessCoalescedPackets()) {
    ...
    //std::cout << "[quic_connection] Call MaybeSendInResponseToPacket() " << "(" << timeStamp() - ho_start_ << "msec)" << std::endl;
...
void QuicConnection::SendAckNow() {
  writer_->SetWritable();
  if (!connected_) {
    return;
  }
  ScopedPacketFlusher flusher(this);
  QuicFrames frames;
  std::cout << "[quic_connection] Send ACK after migration" << std::endl;
  frames.push_back(uber_received_packet_manager_.GetUpdatedAckFrame(
      static_cast<PacketNumberSpace>(APPLICATION_DATA), clock_->ApproximateNow()));
...
void QuicConnection::OnCanWrite() {
  ...
  if (writer_->IsWriteBlocked()) {
    std::cout << "[quic_connection] IsWriteBlocked()" << std::endl;
    const std::string error_details =
  ...
  //std::cout << "[quic_connection] Is True? " << ack_timeout.IsInitialized() << " " << ack_timeout <<"," << clock_->ApproximateNow() << std::endl;

  // ack_timeout.IsInitialized() 얘를 통과 못하고 있었음
  if (ack_timeout.IsInitialized() && ack_timeout <= clock_->ApproximateNow()) {
    ...
    // go off together.
    if (SupportsMultiplePacketNumberSpaces()) {
      //std::cout << "[quic_connection] onCanWrite() => blocked last ack + ack_timeout X => SendAllPendingAcks()" << std::endl;
      SendAllPendingAcks();
    } else {
      SendAck();
    }
  ...
  while (!pending_path_challenge_payloads_.empty()) {
    ...
    //std::cout << "[quic_connection] Send PATH RESPONSE again to " << pending_path_challenge.peer_address << std::endl;
  ...
  //std::cout << "[quic_connection] Hey on can write!" << std::endl;
  visitor_->OnCanWrite();
...
void QuicConnection::WriteIfNotBlocked() {
  ...
  if (!HandleWriteBlocked()) {
    //std::cout << "[quic_connection] write if not blocked()" << std::endl;
    OnCanWrite();
  }
}
...
void QuicConnection::WriteQueuedPackets() {
  ...
  while (!buffered_packets_.empty()) {
    //std::cout << "[quic_connection] WriteQueuedPackets... blocked?" << std::endl;
    if (HandleWriteBlocked()) {
      break;
    }
    ...
    //std::cout << "[quic_connection] Sending buffered packet, result: " << result << std::endl;
...
void QuicConnection::NeuterUnencryptedPackets() {
  ...
  if (SupportsMultiplePacketNumberSpaces()) {
    ...
    // Re-arm ack alarm.
    //std::cout << "[quic_connection] neuter uncrypted packet() -> ack_alarm ???" << std::endl;
...
bool QuicConnection::WritePacket(SerializedPacket* packet) {
  ...
  sent_packet_manager_.SetRetransmittedPacket(stats_.packets_retransmitted);
  ...
  // if (perspective() == Perspective::IS_SERVER) {
  //   std::cout << "[quic_connection] send to address : " << send_to_address << std::endl;
  // }
  ...
  // std::cout << "[quic_connection] Success Write Packet - ";
  ...
  //   std::cout << frame.type << " ";
  // }
  // std::cout << " - " << packet_number << std::endl;
  ...
  if (IsRetransmittable(*packet) == HAS_RETRANSMITTABLE_DATA &&
    ...
    if (!blackhole_detector_.IsDetectionInProgress()) {
      ...
      // std::cout << "[quic_connection] restart blackhole detection after write packet - ";
      // for (const QuicFrame& frame : packet->retransmittable_frames) {
      //   std::cout << frame.type << " ";
      // }
      // std::cout << " | ";
  ...
  SetPingAlarm();
  //std::cout << "[quic_connection] writePacket() => RetirePeerIssuedConnectionIdsNoLongerOnPath()" << std::endl;
  RetirePeerIssuedConnectionIdsNoLongerOnPath();
...
QuicTime QuicConnection::GetPathMtuReductionDeadline() const {
  if (previous_validated_mtu_ == 0) {
    //std::cout << "[quic_connection] get path mtu X" << std::endl;
...
void QuicConnection::OnWriteError(int error_code) {
  ...
  //std::cout << "[quic_connection] Write Error: " << default_path_.self_address << std::endl; 
  const std::string error_details = absl::StrCat(
...
void QuicConnection::OnSerializedPacket(SerializedPacket serialized_packet) {
  ...
  //std::cout << "[quic_connection] check5: OnSerializedPacket" << std::endl;
...
void QuicConnection::OnCongestionChange() {
  ...
  //std::cout << "[quic_connection] congestion change smoothedRtt:" << rtt << " minRtt: " << sent_packet_manager_.GetRttStats()->min_rtt() << "  latestRtt: " << sent_packet_manager_.GetRttStats()->latest_rtt() << "  WindowSize: " << sent_packet_manager_.GetCongestionWindowInTcpMss() << std::endl;
  if (rtt.IsZero()) {
...
void QuicConnection::SendOrQueuePacket(SerializedPacket packet) {
  ...
  //if(!GetHandoverState())
  //std::cout << "[quic_connection] send or queue packet" << std::endl;
  WritePacket(&packet);
}

void QuicConnection::OnPingTimeout() {
  std::cout << "[quic_connection] ping timeout - Has in flight packet? " << sent_packet_manager_.HasInFlightPackets() 
              << " Has pending ack? " << HasPendingAcks() << std::endl;

  std::fstream writer;
  writer.open("ping_timeout.txt", std::ios::app);
  writer << sent_packet_manager_.HasInFlightPackets()  << '\t' << HasPendingAcks() << std::endl;
  writer.close();
...
void QuicConnection::OnRetransmissionTimeout() {
  ...
  // if(!sent_packet_manager_.unacked_packets().HasInFlightPackets()) {
  //   //std::cout << "[quic_connection] HasInFlightPackets is 0, keep going.." << std::endl;
  //   return;
  // }
  ...
  if (sent_packet_manager_.skip_packet_number_for_pto() &&
    ...
    // immediate ACK.
    //std::cout << "[quic_connection] skip packet number" << std::endl;
...
void QuicConnection::SetDefaultEncryptionLevel(EncryptionLevel level) {
  ...
  if (changing_level && packet_creator_.HasPendingFrames()) {
    ...
    ScopedPacketFlusher flusher(this);
    //std::cout << "[quic_connection] SetDefaultEncryptionLevel" << std::endl;
    packet_creator_.FlushCurrentPacket();
  }
...
bool QuicConnection::MaybeProcessCoalescedPackets() {
  ...
  if (processed) {
    MaybeProcessUndecryptablePackets();
    //std::cout << "[quic_connection] QueueCoalescedPacket()" << std::endl;
    MaybeSendInResponseToPacket();
  }
  return processed;
}
...
void QuicConnection::CloseConnection(
  ...
  std::cout << "[quic_connection] Connection Closed.. - " << QuicErrorCodeToString(error) << std::endl;
  // if(perspective_ == Perspective::IS_SERVER) {
  //   std::fstream writer;
  //   writer.open("total_loss.txt", std::ios::app);
  //   writer << maxWindow << '\t' << maxRet << std::endl;
  //   writer.close();
...
void QuicConnection::SendConnectionClosePacket(
  ...
  if (!SupportsMultiplePacketNumberSpaces()) {
    ...
    ScopedPacketFlusher flusher(this);
    //std::cout << "[quic_connection] send cc pacekt" << std::endl;
    ...
    std::cout << "[quic_connection] send connectnion close packet" << std::endl;
...
void QuicConnection::SetRetransmissionAlarm() {
  ...
  if (packet_creator_.PacketFlusherAttached()) {
    ...
    //std::cout << "[quic_connection] pending ret alarm " << std::endl;
    return;
  }
  if (LimitedByAmplificationFactor()) {
...
QuicConnection::ScopedPacketFlusher::~ScopedPacketFlusher() {
  ...
  //std::cout << "[quic_connection] ~packet flusher~~" << std::endl; 
  if (flush_and_set_pending_retransmission_alarm_on_delete_) {
    ...
    if (ack_timeout.IsInitialized()) {
      if (ack_timeout <= connection_->clock_->ApproximateNow() &&
        ...
        //std::cout << "[quic_connection] flusher => ack_alarm update" << std::endl;
    ...
    if (connection_->ack_alarm_->IsSet() &&
      ...
      if (connection_->send_alarm_->IsSet() &&
        ...
        //std::cout << "[quic_connection] flusher => ack_alarm cancel" << std::endl;
        connection_->ack_alarm_->Cancel();
      } else if (connection_->SupportsMultiplePacketNumberSpaces()) {
        //std::cout << "[quic_connection] Flusher ack" << std::endl;
        connection_->SendAllPendingAcks();
      } else {
        connection_->SendAck();
      }
    }
    //std::cout << "[quic_connection]  flush()" << std::endl;
    connection_->packet_creator_.Flush();
    //std::cout << "[quic_connection] end flush()" << std::endl;
    ...
    if (connection_->pending_retransmission_alarm_) {
      //std::cout << "[quic_connection] flusher and ret set for ack_alarm" << std::endl;
      connection_->SetRetransmissionAlarm();
      connection_->pending_retransmission_alarm_ = false;
    }
  }
  //std::cout << "[quic_connection] end flusher~" << std::endl;
...
bool QuicConnection::SendGenericPathProbePacket(
  ...
  if (perspective_ == Perspective::IS_SERVER && probing_writer == nullptr) {
    ...
    //std::cout << "[quic_connection] probing response - writer change" << std::endl;
...
bool QuicConnection::WritePacketUsingWriter(
  ...
  //std::cout << "[quic_connection] check2: write packet using writer" << std::endl;
  WriteResult result = writer->WritePacket(
      packet->encrypted_buffer, packet->encrypted_length, self_address.host(),
      peer_address, per_packet_options_);

  //std::cout << "[quic_connection] WritePacketUsingWriter:" << std::endl;
  for(unsigned long i=0; i<packet->nonretransmittable_frames.size(); i++) {
      std::cout << packet->nonretransmittable_frames[i] << std::endl;
  }
  ...
  if (IsWriteError(result.status)) {
    ...
    QUIC_DLOG(INFO) << ENDPOINT << "Write probing packet failed with error = "
                    << result.error_code;
    //std::cout << "[quic_connection] Write Error" << std::endl;
    return false;
  }
  ...
  if (IsWriteBlockedStatus(result.status)) {
    if (writer == writer_) {
      // Visitor should not be write blocked if the probing writer is not the
      // default packet writer.
      //std::cout << "[quic_connection] write block" << std::endl;
      visitor_->OnWriteBlocked();
    }
    if (result.status == WRITE_STATUS_BLOCKED_DATA_BUFFERED) {
  ...
  //std::cout << "[quic_connection] write true" << std::endl;
  return true;
}
...
void QuicConnection::OnEffectivePeerMigrationValidated() {
  if (active_effective_peer_migration_type_ == NO_CHANGE) {
    ...
    //std::cout << "[quic_connection] No migration underway" << std::endl;
    return;
  }
  highest_packet_sent_before_effective_peer_migration_.Clear();
  //std::cout << "[quic_connection] onEffectivePeerMigrationValidated() Change Type: " << active_effective_peer_migration_type_ << std::endl;

  //std::cout << "[quic_connection] Before send NEW_TOKEN, type is " << active_effective_peer_migration_type_ << std::endl;
  ...
  if (send_address_token) {
    //std::cout << "[quic_connection] Send NEW TOKEN" << std::endl;
...
void QuicConnection::StartEffectivePeerMigration(AddressChangeType type) {
  ...
  std::cout << "[quic_connection] start migration from " << direct_peer_address_ << " to " << current_effective_peer_address << " TYPE: " << active_effective_peer_migration_type_ << std::endl;
  pre_bw_ = BandwidthUnit();
  if(perspective_ == Perspective::IS_SERVER) {
    std::fstream writer;
    writer.open("win_size.txt", std::ios::app);
    writer << sent_packet_manager_.GetCongestionWindowInTcpMss() << std::endl;
    writer.close();
  ...
  if (default_path_.validated) {
    ...
    //std::cout << "[quic_connection] Default path is already validated, maybe send NEW TOKEN" << std::endl;
    OnEffectivePeerMigrationValidated();
    return;
  }
  ...
  if (!path_validator_.IsValidatingPeerAddress(
    ...
    std::cout << "[quic_connection] Path Validation in order to receive Non-Probing Packet" << std::endl;
    ValidatePath(std::make_unique<ReversePathValidationContext>(
    ...
    std::cout << "[quic_connection] Peer address is already under validation - " << default_path_.peer_address << std::endl;
    ++stats_.num_peer_migration_to_proactively_validated_address;
  }
}
...
bool QuicConnection::UpdatePacketContent(QuicFrameType type) {
  ...
  if (version().HasIetfQuicFrames()) {
    if (!QuicUtils::IsProbingFrame(type)) {
      if(current_effective_peer_migration_type_ != NO_CHANGE)
        std::cout << "[quic_connection] Received non-probing " << type << " - prev client ip: " << default_path_.peer_address << " new client ip: " << GetEffectivePeerAddressFromCurrentPacket() << std::endl;
...
void QuicConnection::MaybeStartIetfPeerMigration() {
  ...
  if (GetLargestReceivedPacket().IsInitialized() &&
    ...
    if (current_effective_peer_migration_type_ != NO_CHANGE) {
      ...
      //std::cout << "[quic_connection] Migration(non-probing) type is " << current_effective_peer_migration_type_ << " so, start migration" << std::endl;
      StartEffectivePeerMigration(current_effective_peer_migration_type_);
    } else {
      UpdatePeerAddress(last_received_packet_info_.source_address);
    }
...
void QuicConnection::PostProcessAfterAckFrame(bool send_stop_waiting,
  ...
  if (acked_new_packet) {
    ...
    //std::cout << "[quic_connection] stop detection - post process after ack frame" << std::endl;
    blackhole_detector_.StopDetection(/*permanent=*/false);
  }
...
void QuicConnection::SendAllPendingAcks() {
  ...
  for (int8_t i = INITIAL_DATA; i <= APPLICATION_DATA; ++i) {
    ...
    //std::cout << "[quic_connection] Send ACK" <<  " - me : " << default_path_.self_address << std::endl;
    //std::cout << sent_packet_manager_.BandwidthEstimate() << std::endl;
    ScopedEncryptionLevelContext context(
        this, QuicUtils::GetEncryptionLevel(static_cast<PacketNumberSpace>(i)));
    QuicFrames frames;
    //std::cout << "[quic_connection] Send all pending acks" << std::endl;
    frames.push_back(uber_received_packet_manager_.GetUpdatedAckFrame(
...
//std::cout << "[quic_connection] check3: FlushCoalescedPacket" << std::endl;
  WriteResult result = writer_->WritePacket(
...
void QuicConnection::OnForwardProgressMade() {
  ...
  if (sent_packet_manager_.HasInFlightPackets()) {
    ...
    //std::cout << "[quic_connection] restart detection - on forward progress made" << std::endl;
    blackhole_detector_.RestartDetection(GetPathDegradingDeadline(),
                                         GetNetworkBlackholeDeadline(),
                                         GetPathMtuReductionDeadline());
  } else {
    // Stop detections in quiecense.
    //std::cout << "[quic_connection] stop detection - on forward progress made" << std::endl;
    blackhole_detector_.StopDetection(/*permanent=*/false);
  }
...
QuicTime QuicConnection::GetPathDegradingDeadline() const {
  if (!ShouldDetectPathDegrading()) {
    //std::cout << "[quic_connection] off path degrading deadline" << std::endl;
    return QuicTime::Zero();
  }
  ...
  if (connection_migration_use_new_cid_) {
    {
      ...
      FindOnPathConnectionIds(self_address, effective_peer_address, &client_cid,
                              &server_cid);
      //std::cout << "[quic_connection] my self address port : " << self_address.port() << std::endl;
      // [SD] During PV, make instance cid
      QuicPacketCreator::ScopedPeerAddressContext context(
      ...
      if (writer == writer_) {
        ...
        std::cout << "[quic_connection] Send PC (Non-Probing packet) from:" << self_address << "  to:" << peer_address << " - " << timeStamp() - ho_start_ << "msec" << std::endl;
        packet_creator_.AddPathChallengeFrame(data_buffer);
      } else {
        std::cout << "[quic_connection] Send PC (Probing Packet) from:" << self_address << "  to:" << peer_address << " - " << timeStamp() - ho_start_ << "msec" << std::endl;
        std::unique_ptr<SerializedPacket> probing_packet =
            packet_creator_.SerializePathChallengeConnectivityProbingPacket(
                data_buffer);
...
void QuicConnection::ValidatePath(
  ...
  if (path_validator_.HasPendingPathValidation()) {
    ...
    std::cout << "[quic_connection] cancel path validation" << std::endl;
    path_validator_.CancelPathValidation();
  }
  if (connection_migration_use_new_cid_ &&
      perspective_ == Perspective::IS_CLIENT &&
  ...
  //std::cout << "[quic_connection] Start Validation - " << timeStamp() - ho_start_ << " msec" << std::endl;
...
bool QuicConnection::SendPathResponse(
  ...
  QuicConnectionId client_cid, server_cid;
  //std::cout << "[quic_connection] Try find path cid!" << std::endl;
  if (connection_migration_use_new_cid_) {
    if(path_validator_.GetContext() != nullptr &&
      path_validator_.GetContext()->self_address() == QuicSocketAddress(QuicIpAddress::Any4(), path_validator_.GetContext()->self_address().port())) {
      // [SD] If self_address use default, change self_address to dest_address
      //std::cout << "[quic_connection] WHY....Try find path cid!" << std::endl;
      FindOnPathConnectionIds(path_validator_.GetContext()->self_address(),
                              effective_peer_address, &client_cid, &server_cid);

      std::cout << "[quic_connection] Change new socket self_address from " << path_validator_.GetContext()->self_address() << " to " << last_received_packet_info_.destination_address << std::endl;
      path_validator_.GetContext()->set_self_address(last_received_packet_info_.destination_address);
      alternative_path_.self_address = last_received_packet_info_.destination_address;
  ...
  if (default_path_.self_address ==
    ...
    // socket.
    std::cout << "[quic_connection] Send PR from " << default_path_.self_address << " to " << peer_address_to_send << " - " << timeStamp() - ho_start_ << "msec - Default path" << std::endl;
  ...
  if (!path_validator_.HasPendingPathValidation() ||
    ...
    // socket.
    std::cout << "[quic_connection] Pending PR to " << peer_address_to_send << " me: " << default_path_.self_address << std::endl;
    return true;
  }
  ...
  std::cout << "[quic_connection] Send PR to " << peer_address_to_send  <<  " - " << timeStamp() - ho_start_ << "msec - Alternative Path"  << std::endl;
...
void QuicConnection::SendPingAtLevel(EncryptionLevel level) {
  //std::cout << "[quic_connection] send ping at level" << std::endl;
  ScopedEncryptionLevelContext context(this, level);
...
bool QuicConnection::UpdateConnectionIdsOnClientMigration(
  ...
  if (self_issued_cid_manager_ != nullptr) {
    std::cout << "[quic_connection] Maybe Send New Connetion IDs" << std::endl;
    self_issued_cid_manager_->MaybeSendNewConnectionIds();
    if (!connected_) {
      return false;
    }
  ...
  if ((self_issued_cid_manager_ != nullptr &&
      ...
      (peer_issued_cid_manager_ != nullptr &&
       !peer_issued_cid_manager_->HasUnusedConnectionId())) {
    std::cout << "[quic_connection] cid check fail - " << peer_issued_cid_manager_->HasUnusedConnectionId() << std::endl;
    return false;
  }
  if (self_issued_cid_manager_ != nullptr) {
    std::cout << "[quic_connection] Conesume One Connection ID" << std::endl;
    default_path_.client_connection_id =
        *self_issued_cid_manager_->ConsumeOneConnectionId();
  }
  if (peer_issued_cid_manager_ != nullptr) {
...
void QuicConnection::RetirePeerIssuedConnectionIdsNoLongerOnPath() {
  ...
  if (perspective_ == Perspective::IS_CLIENT) {
    // 패킷을 보낼 때마다 안쓰는 cid는 리타이어하라는 함수를 호출
...
bool QuicConnection::MigratePath(const QuicSocketAddress& self_address,
  ...
  std::cout << "[quic_connection] Migrate to path (" << self_address << ")" << std::endl;
  if (connection_migration_use_new_cid_) {
    if (!UpdateConnectionIdsOnClientMigration(self_address, peer_address)) {
      if (owns_writer) {
        delete writer;
      }
      std::cout << "[quic_connection] Failure migration to " << self_address << std::endl;
      return false;
    }
  ...
  //   for(unsigned long i=0; i<pending_packet_->retransmittable_frames.size(); i++) {
  //     std::cout << pending_packet_->retransmittable_frames[i] << std::endl; 
  //     packet_creator_.AddFrame(pending_packet_->retransmittable_frames[i], RTO_RETRANSMISSION);
  //   }
  //   std::cout << "[quic_connection] non-ret-frame:" << std::endl;
  //   for(unsigned long i=0; i<pending_packet_->nonretransmittable_frames.size(); i++) {
  //     std::cout << pending_packet_->nonretransmittable_frames[i] << std::endl;
  //     packet_creator_.AddFrame(pending_packet_->nonretransmittable_frames[i], NOT_RETRANSMISSION);
  //   }   
  //   //WritePacket(pending_packet_);
  // }
  // if(pending_packet_ != NULL) {
  ...
  //       //packet_creator_.AddFrame(pending_packet_->retransmittable_frames[i], RTO_RETRANSMISSION);
  //     }
  //     buffered_packets_.emplace_back(*pending_packet_, self_address, peer_address);
  //     WriteQueuedPackets()
  // }
...
void QuicConnection::ReversePathValidationResultDelegate::
  ...
  if (connection_->IsDefaultPath(context->self_address(),
    ...
    std::cout << "[quic_connection] Path validation complete, but this path is not default path" << std::endl;
...
void QuicConnection::ReversePathValidationResultDelegate::
  ...
  std::cout << "[quic_connection] path validation fail" << std::endl;
  ...
  // [SD] fail path validation after network is unreachable error
  if(connection_->IsNetworkUnreachable()) {
    std::cout << "[quic_connection] Fail path validation after network is unreachable" << std::endl;
    connection_->OnWriteError(101);
  }
}
